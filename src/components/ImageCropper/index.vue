...
export default class ImageCropper extends Vue {
  ...
  // 不预览圆形图片
  @Prop({ default: false })
  private noCircle!: boolean;
  // 不预览方形图片
  @Prop({ default: false })
  private noSquare!: boolean;
  // 单文件大小限制
  @Prop({ default: 10240 })
  ...
  // 语言类型
  @Prop({ default: null })
  private langExt!: object;
  // 图片上传格式
  @Prop({ default: "png" })
  private imgFormat!: string;
  // 是否支持跨域
  @Prop({ default: false })
  ...
  private get mime() {
    ...
    const tempImgFormat: string =
      allowImgFormat.indexOf(this.imgFormat) === -1 ? "jpg" : this.imgFormat;
    const mime: string = (mimes as any)[tempImgFormat];
    return mime;
  }

  private get lang() {
    const langs = language as any;
    const lang = langs[this.langType] ? langs[this.langType] : langs["en"];
    if (this.langExt) {
  ...
  // 原图地址、生成图片地址
  private sourceImg: any = null;
  private sourceImgUrl: string = "";
  private createImgUrl: string = "";

  // 原图片拖动事件初始值
  private sourceImgMouseDown: any = {
    on: false,
    mX: 0, // 鼠标按下的坐标
    mY: 0,
    x: 0, // scale原图坐标
  ...
  // 原图样式
  private sourceImgStyle() {
    // const scales =this.scale;
  ...
  // 原图蒙版属性
  private sourceImgMasking() {
    const { width, height, ratio, sourceImgContainer } = this;
    const sic: any = sourceImgContainer;
  ...
  sourceImgShadeStyle() {
    ...
    const sic: any = sourceImgContainer;
    const sim: any = sourceImgMasking();
  ...
  @Watch("value")
  private getValue(newValue: number) {
    if (newValue && this.loading != 1) {
      (this as any).reset();
    }
  }
  // 点击波纹效果
  private ripple(e: any) {
    effectRipple(e);
  }
  ...
  // 按钮按下开始向右旋转
  private startRotateRight(e: any) {
    let that = this,
      { scale } = that;
  ...
  // 按钮松开或移开取消放大
  private endZoomAdd(e: any) {
    this.scale.zoomAddOn = false;
  }
  // 按钮按下开始缩小
  private startZoomSub(e: any) {
    let that = this,
      { scale } = that;
  ...
  // 按钮松开或移开取消缩小
  private endZoomSub(e: any) {
    const { scale } = this;
    scale.zoomSubOn = false;
  }
  private zoomChange(e: any) {
  ...
  private createImg(e?: any) {
    ...
    const scale: any = sourceImgMasking().scale;
    ...
    ctx.rotate((Math.PI * this.scale.degree) / 180);
    ctx.translate(-that.width * 0.5, -that.height * 0.5);
    ctx.drawImage(
      sourceImg,
      this.scale.x / scale,
